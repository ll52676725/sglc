...
function getExistingMedia(db) {
  const stmt = db.prepare('SELECT id, type, url, thumbnail_url FROM media')
  const media = []
  while (stmt.step()) {
...
async function generateMockData() {
  ...
  const existingMedia = getExistingMedia(db)
  
  console.log(`现有媒体文件: ${existingMedia.length} 个\n`)
  ...
  for (let i = 0; i < 50; i++) {
    ...
    if (mediaCount > 0 && existingMedia.length > 0) {
      const selectedMedia = getRandomItems(existingMedia, Math.min(mediaCount, existingMedia.length))
  ...
  for (let i = 0; i < 50; i++) {
    ...
    if (existingMedia.length > 0) {
      const selectedMedia = getRandomItems(existingMedia, Math.min(albumMediaCount, existingMedia.length))
    ...
    createdAlbums.push({ id: albumId, name, category })
